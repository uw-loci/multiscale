...
class Plotter(object):
    ...
    def update_plots(self, new_metric_value, transform_parameters):
...
class RegistrationPlot(object):
    def __init__(self, fixed_array, fixed_spacing, fixed_origin, moving_array, moving_spacing, moving_origin):
        ...
        self.plotter = Plotter(fixed_array, fixed_spacing, fixed_origin, moving_array, moving_spacing, moving_origin)
        self.plot_process = mp.Process(target=self.plotter, args=(plotter_pipe,), daemon=True)
        self.plot_process.start()

    def plot(self, new_metric_value, transform_params, finished=False):
        send = self.plot_pipe.send
        if finished:
            send(None)
        else:
            send((new_metric_value, transform_params))
...
if __name__ == '__main__':
    ...
    for i in range(50):
        ...
        reg_plot.plot((-i/50), transform.GetParameters())
